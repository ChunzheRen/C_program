...
{  
    ...
    */  
    ...
    (*L) = (LinkList)malloc( sizeof(struct LNode) );  //LinkList本身是指针类型， 
    (*L) = (LNode*)malloc( sizeof(struct LNode) ); 
    
    
    //首先不是强制类型转化 ，然后LinkList因为是一个别名，所以不需加*,而LNode是类型名，
	//所以需要加上* LNode 
    //2017年9月12日12:38:38 
//   L= (LinkList*) malloc(sizeof(struct LNode)); 
//并不是另外一种解决方法，不可以把地址赋给指针，必须动态分配的内存分配给一个对象，也就是指针指向的对象。 
    //【自己】?????对于L来说自己是一个指针，简单来说就是malloc要匹配。 
    if( !(*L) )  
        exit(0);        // 存储分配失败  
    (*L)->next = NULL;   // 指针域为空  
